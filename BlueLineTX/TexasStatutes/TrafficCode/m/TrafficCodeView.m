...
@implementation TrafficCodeView {
    NSArray * categorey;
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSString* plistPath = [[NSBundle mainBundle] pathForResource:@"TrafficCodeView" ofType:@"plist"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TrafficView *myViewController = [[TrafficView alloc] init];
    NSString *original = [[[categorey objectAtIndex:indexPath.section] objectAtIndex:indexPath.row+1] objectAtIndex:0];
    NSString *trimmed = [original stringByReplacingOccurrencesOfString:@"TrafficCode" withString:@""];
