...
- (void)viewWillAppear:(BOOL)animated {
    ...
    NSString* plistPath = [[NSBundle mainBundle] pathForResource:@"PenalCodeView" ofType:@"plist"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PenalView *myViewController = [[PenalView alloc] init];
    NSString *original = [[[categorey objectAtIndex:indexPath.section] objectAtIndex:indexPath.row+1] objectAtIndex:0];
    NSString *trimmed = [original stringByReplacingOccurrencesOfString:@"PenalCode" withString:@""];
